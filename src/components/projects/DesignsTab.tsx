  ...
  <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
            ...
            return (
              <div key={i} className={`flex items-center justify-between p-2 rounded border text-xs ${isTooLarge ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-100'}`}>
                <div className="truncate flex-1 mr-2 flex items-center gap-1.5 min-w-0">
                  ...
                  <span className={`text-[11px] whitespace-nowrap font-medium ${isTooLarge ? 'text-red-600 font-bold' : 'text-gray-500'}`}>
                    ({sizeMb} MB)
                  </span>
                  {isTooLarge && (
    ...
    <div className="flex gap-3 pt-2">
      ...
      <button
        type="button"
        onClick={onUpload}
        disabled={uploading || uploadForm.files.length === 0 || uploadForm.files.some(f => f.size > 500 * 1024 * 1024)}
...
export function DesignsTab({
  ...
  const handleFileUpload = async () => {
    ...
    const oversizedFiles = uploadForm.files.filter(f => f.size > 500 * 1024 * 1024);
    if (oversizedFiles.length > 0) {
      const first = oversizedFiles[0];
      const sizeMb = (first.size / (1024 * 1024)).toFixed(1);
      setFormError(`File "${first.name}" (${sizeMb} MB) exceeds the 500 MB upload limit.`);
      return;
    }
    setUploading(true);
    setUploadProgress(0);
